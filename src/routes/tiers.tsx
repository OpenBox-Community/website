        ...
        <div className="container">
          <div className="tiers">
            {tiers.map((t) => (
              <div
                key={t.name}
                className={`tier ${"featured" in t && t.featured ? "tier--featured pulse-glow" : ""}`}
              >
                <div className="label">// {t.name.toUpperCase()}</div>
                <div className="tier__name">{t.name}</div>
                <div className="tier__price">
                  <CountUp to={Number(t.price)} prefix="$" />
                </div>
          ...
          >
            ...
            <p style={{ marginTop: 12, color: "var(--text)" }}>
              All Patreon tiers are recurring monthly. Cancel anytime. Roles are auto-assigned via
              the Patreon bot once your Patreon is linked to your Discord account. See{" "}
              <a style={{ color: "var(--green)" }} href="/docs#support-tiers">
                /docs
              </a>{" "}
              for setup.
            </p>
