...
export function BlogContent({ filter }: { filter?: string }) {
  ...
  if (selectedPost) {
    return <PostView post={selectedPost} onClose={closePost} copiedIdx={copiedIdx} handleCopy={handleCopy} />;
  }
export function PostView({ 
  ...
  handleCopy: (code: string, idx: number) => void 
}) {
  return (
    <div className="page">
      <article className="container fade-up" style={{ maxWidth: "800px", paddingBottom: "100px" }}>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "16px" }}>
          ...
          <span className="label" style={{ color: "var(--muted)" }}>{post.readingTime}</span>
        </div>
        ...
        >
          ...
          <div>
            <div className="label" style={{ color: "var(--text)" }}>{post.author.name}</div>
            <div className="label" style={{ fontSize: "9px", color: "var(--muted)" }}>{post.author.role}</div>
          </div>
        </div>
